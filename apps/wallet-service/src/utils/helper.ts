...
import { generateErrorResponse, ResponseData } from "./commonObject";
...
export const convertCoinAmountToInt = (amount: Decimal | number, decimal: number): string => {
...
export const convertCoinAmountFromInt = (amount: string | number, decimal: number): string => {
    decimal = Number(decimal);
    if (isNaN(decimal) || decimal <= 0 ) {
        throw new CustomError(`Invalid decimal value ${decimal} in convertCoinAmountFromInt.`);
    }
...
function concatBuffers(a: any, b: any) {
    return concatTypedArrays(new Uint8Array(a.buffer || a), new Uint8Array(b.buffer || b));
}
...
export const processError = (error: any, res?: Response): ResponseData => {
