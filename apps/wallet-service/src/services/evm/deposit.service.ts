import { deposite_transactionsPayload, networksPayload, PrismaClient } from "@prisma/client";
import Web3 from "web3";
import { generateErrorResponse, generateSuccessResponse } from "../../utils/commonObject";
import { ADDRESS_TYPE_EXTERNAL, EVM_BASE_COIN, NATIVE_COIN, SOLANA_BASE_COIN, TOKEN_COIN, TRON_BASE_COIN } from "../../utils/coreConstant";
import { getLatestTransaction, getLatestWeb3BlockNumber, getPastTransaction, getTransaction } from "./erc20.web3.service";
...
import { checkIfValidTransaction, checkTrxDepositByBlockNumber, convertAddressAmount } from "./trx.tron-web.service";
...
const depositUserWalletBySingleTx = async (
    depositData: Types.GetResult<deposite_transactionsPayload, DefaultArgs>,
    networkData?: Types.GetResult<networksPayload, DefaultArgs>
) => {
    const checkTransaction = await prisma.deposite_transactions.findFirst({
        where: {
            transaction_id: depositData.transaction_id,
            network_id: depositData.network_id,
    ...
    if (checkTransaction) {
        networkData
            ? console.log(
                `[${networkData.slug}]: Tx [${depositData.transaction_id}] block[${depositData.block_number}] already processed`
            )
            : console.log(`Tx [${depositData.transaction_id}] block[${depositData.block_number}] already processed`);
        return;
    }

    const checkSystemWallet = await prisma.admin_wallet_keys.findFirst({
        where: { address: depositData.from_address },
    });

    if (checkSystemWallet) {
        networkData
            ? console.log(
                `[${networkData.slug}]: This Deposit Tx [${depositData.transaction_id}] block[${depositData.block_number}] is sent from system wallet, it will not be considered as user wallet deposit`
            )
            : console.log(
                `This Deposit Tx [${depositData.transaction_id}] block[${depositData.block_number}] is sent from system wallet, it will not be considered as user wallet deposit`
            );
        return;
    }

    if(!await checkTransactionValidity(depositData, networkData)) {
        return;
    }
    ...
    await prisma.$transaction([
        ...
        prisma.wallets.update({
            where: { id: Number(depositData.receiver_wallet_id) },
            data: {
...
const checkTransactionValidity = async (
    transaction: Types.GetResult<deposite_transactionsPayload, DefaultArgs>, 
    networkData: NetworkData
): Promise<boolean> => {
    const transactionId = transaction.transaction_id
    if(Number(networkData.base_type) == SOLANA_BASE_COIN) {
        return true;
    }

    if(Number(networkData.base_type) == EVM_BASE_COIN) {
        const transaction = await getTransaction(networkData.rpc_url, transactionId);
        return transaction?.success;
    }

    if(Number(networkData.base_type) == TRON_BASE_COIN) {
        return await checkIfValidTransaction(networkData.rpc_url, transaction);   
    }
