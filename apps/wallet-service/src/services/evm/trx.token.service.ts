...
import { convertCoinAmountFromInt, convertCoinAmountToInt, powerOfTen, sleep } from "../../utils/helper";
...
const sendTrxToken = async (
  ...
  try {
    ...
    amount = Number(convertCoinAmountToInt(amount, decimalValue));
    ...
    if (transaction) {
      ...
      // if (success) {
        const data = {
          ...
          transaction_id: transaction[0] ? transaction[0] :transaction,
          used_gas: 0,
        };
...
const getTrxEstimateGas = async (
  ...
  try {
    ...
    const parameter = [
      ...
      {
        type: "uint256",
        value: convertCoinAmountToInt(amount,6),
      },
    ];
...
const getDecimalFromContract = async(rpcUrl:string, contact:string): Promise<number | null> => {
    const tronWeb = await initializeTronWeb(rpcUrl);

    if(!tronWeb.isAddress(contact)) {
      return null;
    }

    tronWeb.setAddress(contact);
    const contractInterface = tronWeb.contract(tronABI.entrys,contact);
    const contractDecimal = await contractInterface.decimals().call();

    return contractDecimal;
}

const getCoinDecimalFromContractAddress = async (rpc: string, contractAddress: string): Promise<number> => {
   const tronWeb = await initializeTronWeb(rpc);

    if(!tronWeb.isAddress(contractAddress)) {
      throw new CustomError(`ContractAddress ${contractAddress} is not a valid`);
    }

    tronWeb.setAddress(contractAddress);
    const contractInterface = tronWeb.contract(tronABI.entrys, contractAddress);
    const decimal = await contractInterface.decimals().call();
...
const getTrc20TokenBalance = async(rpcUrl:string, contractAddress:string, address:string)  => {
  try {
      ...
      const balance = convertCoinAmountFromInt(response,decimal);
...
export {
  ...
  getCoinDecimalFromContractAddress,
  getDecimalFromContract
};