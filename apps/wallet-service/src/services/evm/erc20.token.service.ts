...
const getEthTokenBalance = async (rpcUrl: string, address: string, contractAddress: string) => {
    ...
    try {
        ...
        if (tokenBalance) {
            ...
            balance = convertCoinAmountFromInt(tokenBalance, tokenDecimal);
...
const estimateEthTokenFee = async (
    ...
    try {
        ...
        const tokenAmount = convertCoinAmountToInt(amount_value, decimalValue);
        ...
        if (Number(balanceRequired) > Number(balance)) {
            const balanceShortage = minusNumbers(Number(balanceRequired), Number(balance));
      ...
      ${"Try less amount. Or, "}`;
        ...
        const maxFee = convertCoinAmountFromInt(multiplyNumbers(gas_limit, Number(gas_price)), nativeDecimal);
        ...
        if (Number(feeBalanceRequired) > Number(feeBalance)) {
            const feeBalanceShortage = minusNumbers(Number(feeBalanceRequired), Number(feeBalance));
       ...
       ${"Fee balance shortage "}: ${feeBalanceShortage} ${nativeCurrency}`;
        ...
        if (gas > gasLimit) {
            message = `Network is too busy now, Fee is too high. ${"Sending"} ${coinType} ${"coin in"} ${rpcUrl} 
      ${"will ran out of gas. gas needed"}=${gas}, ${"gas limit we are sending"}=${gas_limit}`;
            // console.log(message);
            return generateErrorResponse(message);
        }

        const estimatedFee = convertCoinAmountFromInt(multiplyNumbers(gas, Number(gas_price)), nativeDecimal);
...
const sendErc20Token = async (
    ...
    try {
        ...
        if (validateToAddress) {
            ...
            const initializeContract = await initializeErc20Contact(rpcUrl, contractAddress);
            const decimalValue = await contractDecimal(initializeContract);
            amount = convertCoinAmountToInt(amount_value, decimalValue);
            ...
            const transaction = await executeEthTransaction(tx, web3, pk, coin_type);
...
const getERC20tokenTransactionDetails = async (
    ...
    try {
        ...
        if (checkHash) {
            ...
            if (response) {
                ...
                if (contractAddress) {
                    ...
                    const input = web3.eth.abi.decodeParameters(types, response.data.input.substring(10));
                    ...
                    amount = Number(convertCoinAmountFromInt(amount, tokenDecimal));
                }
        ...
        console.log("getERC20tokenTransactionDetails err: ", err);
        return generateErrorResponse(err.stack);
    }
};

// get contract details
const getContractDetails = async (rpcUrl: string, contractAddress: string) => {
    try {
        const data = {
            chain_id: 0,
            symbol: "",
        ...
        if (result.success == true) {
            ...
            result.data.forEach(function (res: any) {
                let innerData = {
                    ...
                    amount: convertCoinAmountFromInt(res.returnValues.value, decimalValue),
...
const decodeInputParameter = async (rpcUrl: string, contractAddress: string, inputs: any) => {
    try {
        ...
        // Decode the input data using the types of the function arguments
        const input = web3.eth.abi.decodeParameters(types, inputs.substring(10));
        // The amount of tokens transferred is the second parameter
        let amount = input[1];
        let toAddress = input[0];
        const tokenDecimal = await contractDecimal(contract);

        amount = convertCoinAmountFromInt(amount, tokenDecimal);
        const data = {
            amount: amount,
...
const decodeContractInputParameter = async (rpcUrl: string, contractAddress: string, inputs: any, decimal?: number):
    Promise<{
        to_address?: string,
        amount?: number,
        decimal?: number
    ...
    const tokenDecimal = decimal ? decimal : await contractDecimal(contract);
    
    let amount = convertCoinAmountFromInt(data.amount, Number(tokenDecimal));

    data.amount = amount;
    data.decimal = tokenDecimal;
