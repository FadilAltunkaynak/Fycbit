...
import { REGEX, addNumbers, convertCoinAmountFromInt, convertCoinAmountToInt, divideNumbers, minusNumbers, multiplyNumbers, sleep } from "../../utils/helper";
...
const estimateEthFeeWithoutChecking = async (
  ...
  try {
    ...
    // console.log('convertCoinAmountToInt(amount,coinDecimal) =', convertCoinAmountToInt(amount,coinDecimal));
    ;
    ...
    let maxFee = convertCoinAmountFromInt(multiplyNumbers(gasLimit, Number(gasPrice)),coinDecimal);
    ...
    // const estimatedFee = convertCoinAmountFromInt(multiplyNumbers(gas, Number(gasPrice)),coinDecimal);
...
const getTransaction = async(rpcUrl:string,txHash:string): Promise<any> => {
  try {
    ...
    if (!txObj) {
      return generateErrorResponse(`Transaction not found`, { msg_for_user: true });
    }

    const confTxObj = await web3.eth.getTransactionReceipt(txHash);
    if (!confTxObj) {
      return generateErrorResponse(`Transaction not confirmed`, { msg_for_user: true });
    }

    if (!confTxObj?.status) {
      return generateErrorResponse(`This is a failed transaction`, { msg_for_user: true });
    }
    ...
    return generateErrorResponse(e.message ?? `Failed to get transaction information`);
  }
}
  ...
  try {
    ...
    if(contract){
      ...
      tx.data = connectWeb3.eth.abi.encodeFunctionCall({
        ...
        "type": "function"
      }, [toAddress, convertCoinAmountToInt(amount,coinDecimal)]);
    }
    ...
    return generateSuccessResponse('success', {
      fee:estimatedFee //convertCoinAmountToInt(Number(estimatedFee),coinDecimal)
    })
