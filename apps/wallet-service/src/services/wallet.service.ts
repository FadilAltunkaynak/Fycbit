...
import {
    ...
    STATUS_ACTIVE,
    TRON_BASE_COIN,
    WITHDRAWAL_FIXED_FEES,
    WITHDRAWAL_PERCENTAGE_FEES,
...
import { custome_encrypt, custome_decrypt, fees_calculator, generateRandomString, processError } from "../utils/helper";
...
import { createSolanaAddress, validateSolToken } from "./solana/sol.service";
import { sendSolanaNativeCoin } from "./solana/sol.native.service";
import { sendSolanaCoin } from "./solana/sol.token.service";
...
const createAddress = async (user: any, coinType: string, network: number) => {
    ...
    if (getNetwork) {
        ...
        if (getNetwork.base_type == EVM_BASE_COIN) {
            ...
            wallet = await createSolanaAddress(getNetwork.rpc_url ?? "/");
        }

        if (wallet && wallet.success) {
            let walletAddressHistory = await createWalletAddressHistorie(
                Number(User.id),
                coinType,
                Number(getNetwork.id),
                wallet,
                userWallet
            );
...
const createWalletAddressService = async (network: number) => {
    const getNetwork = await getNetworkData(network);

    if (getNetwork) {
        if (!getNetwork.rpc_url?.match(/^http(s)?:\/\/.+/g)) return generateErrorResponse("Invalid RPC url provided");
        let wallet = null;
    ...
    if (getNetwork) {
        ...
        let wallet =
            getNetwork.base_type == EVM_BASE_COIN
                ...
                ? await createSolanaAddress(getNetwork.rpc_url ?? "/")
                : null;
...
const createWalletAddressHistorie = async (
    userId: number,
    coinType: string,
    networkId: number,
    wallet: any,
...
const walletWithdrawalService = async (request: any) => {
    // check base type
    if (!checkBaseType(request.base_type)) return generateErrorResponse("Base type in invalid");
    ...
    if (!coinNetwork.coin_decimal) {
        return generateErrorResponse("Coin decimal is not set for this coin network. Please contact admin");
    }

    const parts = request.amount.toString().split(".");
    if (parts.length > 1 && parts[1].length > coinNetwork.coin_decimal) {
        return generateErrorResponse(
            "Decimal part for amount cannot be more than " + coinNetwork.coin_decimal + " digits for this coin network."
        );
    }
...
const acceptPendingExternalWithdrawal = async (withdrawal_history: any, adminID?: Number) => {
    if (adminID) {
    ...
    let coinNetwork = await prisma.coin_networks.findFirst({
        where: { network_id: Number(network.id), currency_id: Number(coin?.id) },
    });
    if (!adminWallet) {
    ...
    if (!coinNetwork) {
        return generateErrorResponse("Coin network not found");
    }

    if (coinNetwork?.coin_decimal === null) {
        return generateErrorResponse("Coin decimal is not set for this coin network");
    }
    const coinDecimalFromCoinNetwork = Number(coinNetwork.coin_decimal);

    const coinDecimal = Number(coin?.decimal) > 0 ? Number(coin?.decimal) : 18;
    if (
        network &&
        (network.base_type == EVM_BASE_COIN ||
            network.base_type == TRON_BASE_COIN ||
            network.base_type == SOLANA_BASE_COIN)
    ) {
        ...
        if (Number(coinNetwork?.type) == NATIVE_COIN) {
            if (network.base_type == EVM_BASE_COIN) {
                ...
                try {
                    tokenSendResponse = await sendSolanaNativeCoin(
                        network.rpc_url,
                        withdrawal_history.address,
                        withdrawal_history.amount,
                        coinDecimalFromCoinNetwork,
                        adminWallet ? await custome_decrypt(adminWallet.pv) : ""
                    );
                } catch (error: any) {
                    if (error.name == "CustomError" && error.message.includes("Balance is = ")) {
                        console.error(error.stack);
                        return generateErrorResponse("Insufficient balance in system wallet. " + error.message);
                    }
            ...
            if (network.base_type == EVM_BASE_COIN) {
                ...
                try {
                    const nativeCoinDecimal = await prisma.coin_networks.findFirst({
                        select: {
                            coin_decimal: true,
                        },
                        where: {
                    ...
                    tokenSendResponse = await sendSolanaCoin(
                        network.rpc_url,
                        coinNetwork?.contract_address || "",
                        withdrawal_history.address,
                        Number(withdrawal_history.amount),
                        coinDecimalFromCoinNetwork,
                        nativeCoinDecimal?.coin_decimal || Math.log10(LAMPORTS_PER_SOL),
                        adminWallet ? await custome_decrypt(adminWallet.pv) : ""
                    );
                } catch (error: any) {
                    if (error.name == "CustomError" && error.message.includes("Sender token account does not exist")) {
                        console.error(error.stack);
                        return generateErrorResponse(`System wallet does not have any ${currency}`);
                    }

                    if (error.name == "CustomError" && error.message.includes("Current balance")) {
                        console.error(error.stack);
                        return generateErrorResponse(
                            `Insufficient ${currency} balance in system wallet. ` + error.message
                        );
                    }
        ...
        if (tokenSendResponse?.success) {
            await prisma.withdraw_histories.update({
                ...
                data: {
                    ...
                    updated_by: adminID ? Number(adminID) : 0,
            ...
            if (adminID) {
                return generateSuccessResponse("User withdrawal processed successfully.", tokenSendResponse.data ?? []);
            } else {
                return generateSuccessResponse("Pending withdrawal accepted Successfully.", tokenSendResponse.data ?? []);
            }
        } else {
...
const checkWithdrawalValidation = async (request: any, user: any, wallet: any, coin: any) => {
    ...
    if (address) {
        ...
        if (userWallet) {
            ...
            if (userWallet.user_id == user.id) return generateErrorResponse("You can not send to your own wallet!");
            // check coin type
    ...
    if (coin.is_withdrawal != STATUS_ACTIVE)
        return generateErrorResponse(coin.coin_type + " coin is not available for withdrawal right now");
    // check coin minimum withdrawal
    if (coin.minimum_withdrawal > totalAmount)
        return generateErrorResponse(
            "Minimum withdrawal amount " + coin.minimum_withdrawal.toFixed(8) + " " + coin.coin_type
        );
    // check coin maximum withdrawal
    if (coin.maximum_withdrawal < totalAmount)
        return generateErrorResponse(
            "Maximum withdrawal amount " + coin.maximum_withdrawal.toFixed(8) + " " + coin.coin_type
        );
...
const adminAcceptPendingWithdrawal = async (request: any) => {
    let withdrawHistory = await prisma.withdraw_histories.findFirst({
        where: { id: Number(request.id), status: WithdrawalStatus.PROCESSING },
    });
    ...
    if (withdrawHistory.address_type == ADDRESS_TYPE_EXTERNAL) {
        return await acceptPendingExternalWithdrawal(withdrawHistory, request?.admin_id);
    }
...
export const validateToken = async (
    ...
    if (baseType === SOLANA_BASE_COIN) {
        return validateSolToken(contractAddress, rpcUrl);
    }

    if (baseType === TRON_BASE_COIN) {
        return generateSuccessResponse("Valid Token");
    }

    if (baseType === EVM_BASE_COIN) {
        return generateSuccessResponse("Valid Token");
    }
