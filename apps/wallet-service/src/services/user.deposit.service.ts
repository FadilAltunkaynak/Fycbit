...
import {
    ...
    getContractDetails as getEvmContractDetails,
    estimateEthTokenFee,
    estimateEthTokenFeeWithoutChecking,
    getEthTokenBalance,
    getAddressByKey,
} from "./evm/erc20.token.service";
import {
    sendTrxCoin,
    getTrxBalance,
...
const receiveDepositCoinProcess = async (request: any) => {
    try {
        if (request.transaction_id) {
            let transaction = await prisma.deposite_transactions.findFirst({
                where: {
                    AND: {
                        id: Number(request.transaction_id),
                        status: STATUS_ACTIVE,
                        is_admin_receive: DepositCollectionStatus.PROCESSING,
                    },
                },
            });
            ...
            if (transaction && transaction.network_id) {
                ...
                const nativeCoinNetworkDecimal = await prisma.coin_networks.findFirst({
                    select: {
                        coin_decimal: true,
                    },
                    where: {
                ...
                if (coinNetwork.length) {
                    if (coinNetwork[0]?.coin_decimal === null) {
                        return generateErrorResponse("Currency decimal for this coin network is not set");
                    }

                    const supported_network: any = await prisma.supported_networks.findFirst({
                        where: { AND: { slug: coinNetwork[0].slug } },
                    });
                    ...
                    coinNetworkData.decimal = Number(coinNetwork[0]?.coin_decimal);
                    ...
                    if (coinNetworkData.base_type == SOLANA_BASE_COIN) {
                        try {
                            return await sendToSolanaSystemWallet(
                                getPendingDepositCallables(coinNetworkData.contractAddress),
                                coinNetwork[0].rpc_url,
                                userWalletAddress.address,
                                systemWallet.address,
                                coinNetworkData.amount,
                                coinNetworkData.decimal,
                                nativeCoinNetworkDecimal?.coin_decimal || null,
                                await custome_decrypt(userWalletAddress.wallet_key),
                                await custome_decrypt(systemWallet.pv),
                                coinNetworkData,
                                coinNetworkData.contractAddress,
                                Number(userWalletAddress.wallet_id),
                                coinNetworkData.coin_type
                            );
                        } catch (error: any) {
                            if (
                                error.name == "CustomError" &&
                                error.message.includes("Sender token account does not exist")
                            ) {
                                console.error(error.stack);
                                return generateErrorResponse(
                                    `User account does not have any ${coinNetworkData.coin_type}`
                                );
                            }
    ...
    if (network.base_type == EVM_BASE_COIN) {
        if (network.is_native) {
            ...
            if (network.amount > userBalance) {
                return generateErrorResponse(
                    "User address has not enough coin. current network balance is " + userBalance
                );
            }
        } else {
            // token
            ...
            let userTokenBalance = await getEthTokenBalance(
                network.rpc_url,
                userWallet.address,
            ...
            if (network.amount > Number(userTokenBalance.data)) {
                return generateErrorResponse(
                    "User address has not enough token. current network balance is " + userTokenBalance.data
                );
            }
        }
        ...
        const transaction = await prisma.deposite_transactions.update({
            ...
            data: {
                status: STATUS_ACTIVE,
                is_admin_receive: DepositCollectionStatus.SUCCESS,
                //transaction_id: sendToWystemWallet.data.transaction_id,
                reject_note: null,
            },
        });
...
const takeCoinFromTronNetwork = async (network: any, systemWallet: any, userWallet: any): Promise<any> => {
    ...
    try {
        ...
        if (
            !(
                userWalletBalance.hasOwnProperty("success") &&
                (userWalletBalance.success || userWalletBalance.data === 0)
            )
        ) {
            return generateErrorResponse(userWalletBalance?.message ?? "User wallet balance check failed");
        }
        ...
        if (network.is_native) {
            ...
            // check token balance
            const checkTokenBalance = await getTrc20TokenBalance(
                network.rpc_url,
                network.contractAddress,
                userWallet.address
            );
            ...
            if (network.amount > checkTokenBalance.data) {
                return generateErrorResponse(
                    "Insufficient token balance , current balance is " + checkTokenBalance.data
                );
            }
        }
        ...
        if (!network.is_native) {
            ...
            if (
                !(
                    userWalletBalanceAgain.hasOwnProperty("success") &&
                    (userWalletBalanceAgain.success || userWalletBalanceAgain.data === 0)
                )
            ) {
...
const checkDepositByTxService = async (request: any) => {
    ...
    if (coin_network.type == NATIVE_COIN) {
        ...
        if (!coin_network.contract_address) return generateErrorResponse("Contact address not found");
    ...
    if (network.base_type == EVM_BASE_COIN) {
        let response = await getERC20tokenTransactionDetails(
            network?.rpc_url ?? "",
            request.transaction_id,
            coin_network.contract_address,
            coin.decimal
        );
        // console.log("EVM_BASE_COIN getERC20tokenTransactionDetails res:", response);
        if (response.hasOwnProperty("success") && response.success) {
            ...
            console.log("evm transaction response:", response);
            if (response?.data?.msg_for_user && response?.message) {
    ...
    if (network.base_type == TRON_BASE_COIN) {
        let response = await getTrxTransactionBlock(
            coin_network.type,
            network?.rpc_url ?? "",
            request.transaction_id,
            coin_network.contract_address,
        ...
        if (response.hasOwnProperty("success") && response.success) {
            ...
            console.log("tron transaction response:", response);
            if (response?.data?.msg_for_user && response?.message) {
                return generateErrorResponse(response.message);
            }
        }
    ...
    if (network.base_type == SOLANA_BASE_COIN) {
        try {
            let transactionInfo = await getSolanaTransferFromTransactionHash(
                network?.rpc_url ?? "",
                Number(coin.id),
                Number(network.id),
                request.transaction_id,
                getTransactionProcessor(coin_network.contract_address),
                coin_network.coin_decimal,
                coin_network.contract_address
            );

            return generateSuccessResponse("Successfully fetched transaction", {
                hash: transactionInfo.transaction_id,
                gas_used: transactionInfo.fee_limit,
                txID: transactionInfo.transaction_id,
                amount: transactionInfo.amount,
                toAddress: transactionInfo.to_address,
                fromAddress: transactionInfo.from_address,
                coin_type: coin.coin_type,
                address: transactionInfo.to_address,
                from: transactionInfo.from_address,
                block_number: transactionInfo.block_number,
                confirmations: 1,
            });
        } catch (error: any) {
            return processError(error);
...
const checkContractInfo = async (request: any) => {
    try {
        ...
        if (contractDetails) return generateSuccessResponse("Contract details get successfully", contractDetails);
