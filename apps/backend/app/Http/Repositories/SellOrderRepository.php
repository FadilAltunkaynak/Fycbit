...
class SellOrderRepository extends CommonRepository implements SellOrderRepositoryInterface
{
    ...
    /**
     * btc_rate as percentage and btc_rate as is_favorite added just to have that on result, so api response not get break on mobile app
    ...
    public function getAllOrders($base_coin_id, $trade_coin_id)
    {
        if (true || (env('APP_ENV') == 'local') || (env('APP_ENV') == 'dev') || (env('APP_ENV') == 'production')) {
            if (getUserId()) {
                $filterSells = DB::table(DB::raw('(select base_coin_id,trade_coin_id, TRUNCATE(price,8) as price, visualNumberFormat(TRUNCATE(sum(amount-processed),8)) as amount, visualNumberFormat(TRUNCATE(sum((amount - processed) * price), 8)) as total, status, processed, created_at, 0 as percentage, 0 as is_favorite
                                        from sells
                    ...
                    ->select('t1.created_at', 't1.status', 't1.processed', 't1.price', 't1.amount', 't1.total', 't2.amount as my_size')
                ...
                $filterSells = DB::table(DB::raw('(select base_coin_id,trade_coin_id, TRUNCATE(price,8) as price, visualNumberFormat(TRUNCATE(sum(amount-processed),8)) as amount, visualNumberFormat(TRUNCATE(sum((amount - processed) * price), 8)) as total, status, processed, created_at, 0 as percentage, 0 as is_favorite
                                        ...
                                        group by base_coin_id,trade_coin_id,price)
                                     t1'))
                    ->select('t1.created_at', 't1.status', 't1.processed', 't1.price', 't1.amount', 't1.total', DB::raw('0 as my_size'))
                    ->orderBy('t1.price', 'ASC');
            }
        }
    ...
    {
        return Sell::withTrashed()->where(['user_id' => $userId, 'base_coin_id' => $baseCoinId, 'trade_coin_id' => $tradeCoinId])
            ->select(
                DB::raw("'sell' as type"),
                'id',
                ...
                DB::raw('visualNumberFormat(TRUNCATE((amount), 8)) as amount, visualNumberFormat(TRUNCATE((amount) * price, 8)) as total,
                    (case when (maker_fees > taker_fees)
                     THEN
                          visualNumberFormat(TRUNCATE((((amount)) * maker_fees) * 0.01, 8))
                     ELSE
                          visualNumberFormat(TRUNCATE((((amount)) * taker_fees) * 0.01, 8))
                     END)
                     as fees')
    ...
    public function getMyOrders($baseCoinId, $tradeCoinId, $userId)
    {
        //        if(getUserId()) {
        if ($userId) {
            return Sell::where(['user_id' => $userId, 'base_coin_id' => $baseCoinId, 'trade_coin_id' => $tradeCoinId, 'status' => 0])
                ->select(
                    DB::raw("'sell' as type"),
                    'id',
                    DB::raw("visualNumberFormat(price) as price"),
                    ...
                    (case when (maker_fees > taker_fees)
                     THEN
                          visualNumberFormat(TRUNCATE((((amount - processed)) * maker_fees) * 0.01, 8))
                     ELSE
                          visualNumberFormat(TRUNCATE((((amount - processed)) * taker_fees) * 0.01, 8))
                     END)
                     as fees')
                );
        } else {
            return Sell::where(['user_id' => 0, 'base_coin_id' => $baseCoinId, 'trade_coin_id' => $tradeCoinId, 'status' => 0])
    ...
    public function getOnOrderBalanceByTradeCoinId(int $coinId, ?int $userId = null): ?string
    {
        $total = DB::table('sells')
            ...
            ->select(DB::raw('TRUNCATE(SUM((sells.amount - sells.processed)), 8) as total'))->first()->total;
