...
class BuyOrderRepository extends CommonRepository implements BuyOrderRepositoryInterface
{
    ...
    {
        return Buy::withTrashed()->where(['user_id' => $userId, 'base_coin_id' => $baseCoinId, 'trade_coin_id' => $tradeCoinId])
            ->select(
                DB::raw("'buy' as type"),
                'id',
    ...
    public function getAllOrders($base_coin_id, $trade_coin_id)
    {
        if (true || (env('APP_ENV') == 'local') || (env('APP_ENV') == 'dev') || (env('APP_ENV') == 'production')) {
            if (getUserId()) {
                $filterBuys = DB::table(DB::raw('(select base_coin_id,trade_coin_id, TRUNCATE(price,8) as price, visualNumberFormat(TRUNCATE(sum(amount-processed),8)) as amount, visualNumberFormat(TRUNCATE(sum((amount - processed) * price), 8)) as total, status, processed, created_at, 0 as percentage, 0 as is_favorite
                                        from buys
    ...
    public function getMyOrders($baseCoinId, $tradeCoinId, $userId)
    {
        //        if(getUserId()) {
        if ($userId) {
            return Buy::where(['user_id' => $userId, 'base_coin_id' => $baseCoinId, 'trade_coin_id' => $tradeCoinId, 'status' => 0])
                ->select(
                    DB::raw("'buy' as type"),
                    'id',
                    DB::raw("visualNumberFormat(price) as price"),
            ...
            return Buy::where(['user_id' => 0, 'base_coin_id' => $baseCoinId, 'trade_coin_id' => $tradeCoinId, 'status' => 0])
