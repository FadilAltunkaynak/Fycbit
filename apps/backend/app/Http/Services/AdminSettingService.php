...
class AdminSettingService extends CommonService
{
    ...
    private SpotCoinPairCacheService $spotCoinPairCacheService;
    ...
    public function __construct()
    {
        ...
        $this->spotCoinPairCacheService = app()->make(SpotCoinPairCacheService::class);
    }
    ...
    public function savePairSetting($request)
    {
        ...
        try {
            ...
            if (isset($request->edit_id)) {
                $editId = decrypt($request->edit_id);
                $existingPair = CoinPair::find($editId);
                if (isset($coinPair) && ($coinPair->id != decrypt($request->edit_id))) {
                    return ['success' => false, 'message' => __('This coin pair already exist')];
                }

                $updated = $setting_repo->updateOrCreateCoinPair($request, $request->edit_id);
                if ($updated && $existingPair) {
                    $this->spotCoinPairCacheService->syncCoinPair(
                        CoinPair::findOrFail($editId),
                        [
                            'child_coin_id' => $existingPair->child_coin_id,
                            'parent_coin_id' => $existingPair->parent_coin_id,
                        ]
                    );
                }
                ...
                if ($a) {
                    $this->spotCoinPairCacheService->syncCoinPair($a);
    ...
    public function changeCoinPairStatus($request)
    {
        try {
            ...
            if (isset($pair)) {
                ...
                $this->spotCoinPairCacheService->syncCoinPair($pair->fresh());
                $success = true;
