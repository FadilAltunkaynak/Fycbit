...
class OtherSpotTradeWebsocketService implements WebsocketDataSenderInterface
{
    ...
    public function sendData(Model $order) 
    {
        ...
        $socketData['pairs'] = app('spot_coinpair_cache')->getCoinPairs()['data'];
        $socketData['trades'] = app('spot_transaction_cache')->getTransactions((object) $request)['data'];
        $socketData['last_trade'] = app('spot_transaction_cache')->getLastTransaction((object) $request)['data'];
