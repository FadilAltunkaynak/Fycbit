...
class WalletService
{
    ...
    /**
     * user wallet network list
     * @param string $coinType
     * @param int|null $type
     * @return array
     */
    public function userWalletNetwork(string $coinType, ?int $type = null): array
    {
        $coin = Coin::when($type, function ($query, $type) {
            return match ($type) {
                DEPOSIT => $query->where('is_deposit', STATUS_ACTIVE),
                WITHDRAWAL => $query->where('is_withdrawal', STATUS_ACTIVE),
            };
        })->where(['coin_type' => $coinType, 'status' => STATUS_ACTIVE])->first();
        ...
        if($user?->role == USER_ROLE_USER){
            $wallet = $this->repository->walletInfo($user?->id ?? 0, $coin);
            if (empty($wallet))
        ...
        if ($coin->coin_type == COIN_USDT && CoinProvider::isCoinPayment($coin->active_provider)) {
            $coinPaymentNetworks = (new WalletAddressHistoryService)
                ->coinPaymentNetworks($coin->coin_type, $coin->active_provider)['data'];

            if (empty($coinPaymentNetworks))
                return failed(__('Coin payment networks not found'));

        } else {
            $networks = CoinNetwork::with('network')
                ->whereHas('network', function ($query) use($coin) {
                    $query->where('status', STATUS_ACTIVE)
                        ->where('provider_type', $coin->active_provider);
                })
                ->where(['currency_id' => $coin->id, 'status' => STATUS_ACTIVE])->get()
                ->map(function ($item) use($coin) {
                    return [
                        'id' => $item->network->id,
                        'name' => $item->network->name,
                        'provider_type' => $item->network->provider_type,
                        'active_provider' => $coin->active_provider,
                    ];
                });
    ...
    /**
     * Summary of walletDepositAddress
     * @param int $userId
     * @param Request $request
     * @return array
     */
    public function walletDepositAddress(int $userId, Request $request): array
    {
        $result = [
            'address' => null,
            'memo' => null,
        ...
        if ($coin->is_deposit == STATUS_INACTIVE)
            return failed(__('Deposit is disable right now'));

        $wallet = $this->repository->walletInfo($userId, $coin);
        if (empty($wallet))
            return failed(__('Wallet not found'));

        $walletAddressService = new WalletAddressHistoryService;

        if ($request->network_type && $coin->coin_type == COIN_USDT && CoinProvider::isCoinPayment($coin->active_provider)) {
            $coinPaymentAddress = $this->getCoinPaymentUsdtAddress($coin, $request->network_type, $wallet);

            if (!is_success($coinPaymentAddress))
                return $coinPaymentAddress;

            $result['address'] = $coinPaymentAddress['data']->address ?? null;
            $result['rented_till'] = $coinPaymentAddress['data']->rented_till ?? null;
            $result['current_time'] =  Carbon::now();
        ...
        $coinNetwork = CoinNetwork::where(['currency_id' => $coin->id, 'network_id' => $network->id])->first();
        if (empty($coinNetwork))
            return failed(__('Coin network not found'));

        $walletAddress = $walletAddressService->getUserWalletAddress($userId, $coin, $network, $wallet->id)['data'];

        $result = [
            'address' => $walletAddress->address,
            'memo' => $walletAddress->memo,
            'token_address' => $coinNetwork->contract_address ?: null
        ];
        return success($result);
    ...
    /**
     * Summary of getCoinPaymentUsdtAddress
     * @param Coin $coin
     * @param string $network_type
     * @param Wallet $wallet
     * @return array
     */
    public function getCoinPaymentUsdtAddress(Coin $coin, string $network_type, Wallet $wallet): array
    {
        if ($coin->coin_type != 'USDT')
            return failed(__('No need to create address with this coin'));

        $networkAddress = WalletNetwork::firstOrCreate(
            [
                'wallet_id' => $wallet->id,
                'network_type' => $network_type,
                'status' => WalletNetworkStatus::ACTIVE->value
            ],
            ['coin_id' => $wallet->coin_id]
        );
        ...
        if (empty($networkAddress->address)) {
            $address = (new WalletAddressHistoryService)->getCoinPaymentWalletAddress($networkAddress->network_type);
        ...
        if (empty($networkAddress->address))
            return failed(__('Failed to generate address'));

        return success($networkAddress);
    }
    ...
    public function getWalletDetailsApp(Request $request): array
    {
        try {
            if (!isset($request->coin_id))
                return responseData(false, __("Coin id is required"));

            if (!($coin = Coin::with(['coin_network', 'coin_network.network:id,name,base_type'])->where('id', $request->coin_id)->first()))
                return responseData(false, __("Coin not found"));

            if (!($wallet = Wallet::where(['coin_id' => $request->coin_id, 'user_id' => getUserId()])->first()))
                return responseData(false, __("Wallet not found"));

            if ($wallet->is_deposit != STATUS_ACTIVE)
                return responseData(false, __("Deposit is disable right now"));

            $networks = [];
            if (isset($coin?->coin_network)) {
                $coin_network = $coin?->coin_network;
                $coin_network->map(function ($coin_network) use (&$networks) {
                    if (isset($coin_network?->network))
                        $networks[] = $coin_network?->network;
                });
            }
            $networks = collect($networks);

            $data['networks'] = $networks;
            $data['address'] = WalletAddressHistory::where([
                'user_id' => getUserId(),
                'coin_type' => $request->coin_type,
            ])->get(['network_id', 'address']);

            return [
                'success' => true,
                'wallet' => $wallet,
                'message' => __('Wallet details get successfully'),
                'data' => $data
            ];
        } catch (\Exception $e) {
            storeException("getWalletDetailsApp", $e->getMessage());
            return responseData(false, __("Something went wrong"));
        }
    }

    public function walletWithdrawDetailsApp(int $wallet_id): array
    {
        try {
            $wallet = Wallet::join('coins', 'coins.id', '=', 'wallets.coin_id')
                ->where(['wallets.id' => $wallet_id, 'wallets.user_id' => getUserId()])
                ->with(['coin', 'coin.coin_network', 'coin.coin_network.network:id,name,base_type'])
                ->select(
                    'wallets.*',
            ...
            if ($wallet->is_withdrawal == STATUS_INACTIVE)
                return responseData(true, __("Wallet is currently disable"));

            $networks = [];
            if (isset($wallet?->coin?->coin_network)) {
                $coin_network = $wallet?->coin?->coin_network;
                $coin_network->map(function ($coin_network) use (&$networks) {
                    if (isset($coin_network?->network))
                        $networks[] = $coin_network?->network;
                });
            ...
            if ($wallet->coin_type == COIN_USDT && $wallet->network == COIN_PAYMENT) {
                $data['coin_payment_networks'] = (new WalletRepository)->depositNetwotkAddress($wallet)['data'];
            }
            ...
            storeException("walletWithdrawDetailsApp", $e->getMessage());
            return responseData(false, __("Something went wrong"));
        }
    }
    ...
    public function walletHistoryApp(Request $request, int $userId): array
    {
        ...
        $data['status'] = DepositeStatus::statusListHtml();
    ...
    public function walletFundTransfer(FundTransferSubmitRequest $request)
    {
        // Check Coin
        $coin = Coin::where('coin_type', $request->coin_type)->first();
        if(!$coin) return failed(__('Coin not found'));

        // Build FundEnum From Params
        $fromFund = FundEnum::tryFrom($request->fund_from);
        $toFund   = FundEnum::tryFrom($request->fund_to);

        if(!$fromFund || !$toFund) return failed(__('Invalid Fund type'));
        if($fromFund == $toFund)   return failed(__('Same Fund type'));

        $service = new FundTransferService();

        // Check From Wallet
        $fromWallet = $service->checkWalletAndBalance(
            fundType: $fromFund,
            params: new FundCheckerQueryParams(
                user_id: authId(),
                coin: $coin,
                from: true
            )
        ...
        if(bccompx($fromWallet->balance , $request->amount) == -1) // balance < amount
            return failed(__('Insufficient from fund balance'));

        // Check To Wallet
        $toWallet = $service->checkWalletAndBalance(
            fundType: $toFund,
            params: new FundCheckerQueryParams(
                user_id: authId(),
                coin: $coin,
        ...
        // Send Balance From One Wallet To Another
        try {
            DB::beginTransaction();
            $took_balance = $service->takeFundFrom(
                fundType: $fromFund,
                params: new FundManageParams(
                    user_id: authId(),
            ...
            if(!$took_balance) {
                DB::rollBack();
                storeException("Wallet From Fund Taker", "Failed deduction");
                return failed(__("Failed to transfer fund from {$fromWallet->name} to {$toWallet->name}"));
            }

            $given_balance = $service->giveFundTo(
                fundType: $toFund,
                params: new FundManageParams(
                    user_id: authId(),
            ...
            if(!$given_balance) {
                DB::rollBack();
                storeException("Wallet To Fund Giver", "Failed Increment");
                return failed(__("Failed to transfer fund from."));
            }

            DB::commit();
            return success(__("Successfully transferred fund."));

        } catch (\Throwable $th) {
            DB::rollBack();
            storeException("Wallet From Fund Transfer", $th->getLine());
            storeException("Wallet From Fund Transfer", $th->getMessage());
