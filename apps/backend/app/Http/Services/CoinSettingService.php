...
class CoinSettingService extends BaseService
{
    ...
    public function updateCoinSetting($request)
    {
        ...
        $network = Network::find(decrypt($request->coin_network_id));
        if (empty($network))
            return failed(__('Network not found'));

        $this->object->createCoinSetting($coin->id, $network->id);

        return match ($network->base_type) {
            NetworkBase::BITGO_API->value => $this->updateBitgoApi($coin->id, $network->id, $request),
            NetworkBase::BITCOIN_API->value => $this->updateBitCoinApi($coin->id, $network->id, $request),
            default => $this->updateERCCoinApi($coin->id, $network->id, $request)
        };
    }
    ...
    public function updateBitCoinApi(int $coinId, int $network, Request $request)
    {
        ...
        return $this->responseData(true, __('Bitcoin Coin api setting updated successfully'));
    }
    ...
    public function updateBitgoApi(int $coinId, int $network, Request $request)
    {
        ...
        $this->object->updateWhere(['coin_id' => $coinId, 'network' => $network], $data);
        return $this->responseData(true, __('Bitgo Coin api setting updated successfully'));
    }
    // update erc20 or bep20 api
    public function updateERCCoinApi(int $coinId, int $network, Request $request)
    {
        $data = [
    ...
    public function storeERCorBEPCoinsettings($data)
    {
        try {

            $response = $this->object->storeERCorBEPCoinsettings($data);
            $response = ['success' => true, 'message' > __('Coin api setting updated successfully')];
        } catch (\Exception $e) {
            storeException('storeERCorBEPCoinsettings', $e->getMessage());
            $response = responseData(false);
        }
        return $response;
    }
