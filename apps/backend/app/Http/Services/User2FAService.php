...
class User2FAService extends BaseService
{
    ...
    public function userGoogle2faValidation($user, $request)
    {
        ...
        if (empty($user->google2fa_secret))
            throw new Exception(__('Enable Your google2fa first'));
        else {
            if (!empty($request->code)) {
                ...
                if ($valid)
                    ...
                    throw new Exception(__('Verify code is invalid'));
            } else
                throw new Exception(__('Verify code is required'));
        }
    }
    ...
    public function checkTwoFactor($request, $type, $userId)
    {
        $settings = settings();

        if (!filter_var($settings[$type] ?? false, FILTER_VALIDATE_BOOLEAN))
            return failed(__("Two factor verification failed!"));

        $user = User::find($userId);
        $response = $this->userOtpVerification($request, $user);
        return $response;
    }
}
