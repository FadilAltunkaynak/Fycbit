...
class CurrencyDepositService extends BaseService
{
    ...
    // deposit with stripe
    public function depositWithStripe($data, $currency = "usd")
    {
        try {
            storeException('stripe amount', $data['amount']);
            $makeAmount = $data['amount'] * 100;
            storeException('stripe make amount', $makeAmount);
