...
class OtherSettingService
{
    ...
    public function deleteWalletAddress(Request $request, User $admin)
    {
        ...
        DB::beginTransaction();
        try {
            WalletAddressHistory::where($conditions)->update(['status' => WalletAddressStatus::EXPIRE->value]);
            DepositeTransaction::where($conditions)->update(['status' => DepositeStatus::EXPIRE->value]);
            WithdrawHistory::where($conditions)->update(['status' => WithdrawStatus::EXPIRE->value]);

            if ($request->coin_type == COIN_USDT && $coin->network == COIN_PAYMENT)
                WalletNetwork::where('coin_id', $coin->id)->update(['status' => WalletNetworkStatus::EXPIRE->value]);
