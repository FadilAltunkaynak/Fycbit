...
class LandingController extends Controller
{
    ...
    private function isLandingFeatureTitleOnlyUpdate(Request $request): bool
    {
        return $request->filled('landing_feature_title')
            && !isset($request->landing_title)
            && !isset($request->landing_description)
            && !$request->hasFile('landing_banner_image');
    }

    private function landingSettingRedirect(Request $request, string $flashKey, string $message)
    {
        if ($this->isLandingFeatureTitleOnlyUpdate($request)) {
            return redirect()->route('adminLandingSetting', ['tab' => 'feature'])->with([$flashKey => $message]);
        }

        return redirect()->route('adminLandingSetting', [
            'tab' => $request->tab ?? 'hero' //$this->landingTab($request)
...
//    Landing Settings
    ...
    public function adminLandingSettingSave(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $data['message'] = $errors;
            return $this->landingSettingRedirect($request, 'dismiss', $errors[0]);
        }
//        $key_data = ['landing_title','landing_description','landing_feature_title','landing_banner_image'];
        foreach ($request->except('_token', 'tab') as $key => $item) {
//            if (in_array($key,$key_data)) {
        ...
        return $this->landingSettingRedirect($request, 'success', __('Landing Page Setting Successfully Updated!'));
    ...
    public function adminFeatureList(Request $request)
    {
        ...
        return redirect()->route('adminLandingSetting', ['tab' => 'feature']);
    }

    // View Add new
    public function adminFeatureAdd()
    {
        $data['title']=__('Add Feature');
    ...
    public function adminFeatureSave(Request $request)
    {
        ...
        try {
            ...
            if ($response['success'] == true) {
                return redirect()->route('adminLandingSetting', ['tab' => 'feature'])->with(['success'=> $response['message']]);
            } else {
    ...
    {
        $response = $this->landingService->adminLandingApiLinkSave($request->except('_token', 'tab'));
        if ($response['success']) {
            return redirect()->route('adminLandingSetting',['tab' => $request->tab ?? 'links'])->with(['success' => $response['message']]);
        } else {
            return redirect()->route('adminLandingSetting',['tab' => $request->tab ?? 'links'])->with(['dismiss'=> $response['message']]);
        }
    }

    public function adminLandingPairAssetSave(Request $request)
    {
        $response = $this->landingService->adminLandingPairAssetSave($request->except('_token', 'tab'));
        if ($response['success']) {
            return redirect()->route('adminLandingSetting',['tab' => $request->tab ?? 'pair_assets'])->with(['success' => $response['message']]);
        } else {
            return redirect()->route('adminLandingSetting',['tab' => $request->tab ?? 'pair_assets'])->with(['dismiss'=> $response['message']]);
        }
    }

    public function adminLandingSectionSettingsSave(Request $request)
    {
        $response = $this->landingService->adminLandingSectionSettingsSave($request->except('_token', 'tab'));
        if ($response['success']) {
            return redirect()->route('adminLandingSetting',['tab' => $request->tab ?? 'section_settings'])->with(['success' => $response['message']]);
        } else {
            return redirect()->route('adminLandingSetting',['tab' => $request->tab ?? 'section_settings'])->with(['dismiss'=> $response['message']]);
        }
    }

    public function adminFooterColumnHeader(Request $request)
    {
        if (isset($_GET['tab'])) {
            $data['tab']=$_GET['tab'];
        } else {
            $data['tab']='footer_column_header';
        }
        $data['settings'] = allsetting();

        return view('admin.settings.landing.footer_column_header',$data);
    }
}
