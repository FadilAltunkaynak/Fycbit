...
class TransactionController extends Controller
{
    ...
    public function adminDepositDetails($id)
    {
        ...
        $networkName = (new CoinSettingRepository())->getNetworkName($transaction->coin->id, $transaction?->network ?? $transaction?->network_id ?? 0);
    ...
    {
        if ($request->ajax())
            return (new WithdrawalHistoryDataTableService())->getData(WithdrawStatus::PENDING->value);
    ...
    {
        if ($request->ajax())
            return (new WithdrawalHistoryDataTableService())->getData(WithdrawStatus::FAILED->value);
    ...
    public function adminAcceptPendingWithdrawal($id)
    {
        ...
        $transaction = WithdrawHistory::with(['wallet', 'users'])
            ->where('id', $wdrl_id)
            ->whereIn('status', [WithdrawStatus::PENDING->value, WithdrawStatus::FAILED->value])
            ->first();

        if (empty($transaction))
            ResponseFacade::failed(__('Transaction not found'))->safeThrow();
        ...
        return ResponseFacade::success(__('Pending withdrawal process started successfully. Be patient and update withdrawal after a while'))->send();
    }
    ...
    {
        $wdrl_id = decrypt($id);
        $transaction = WithdrawHistory::with(['coin', 'senderWallet.user', 'receiverWallet.user', 'network'])->find($wdrl_id);
        ...
        $networkName = $transaction->getRelationValue('network')?->name ?? __("Not Found");
        ...
        try {
            if (isset($request->id)) {
                ...
                if (!isset($request->reject_note))
                    return redirect()->back()->with('dismiss', __('Reject not required'));
                if ($deposit = CurrencyDepositHistory::where(['id' => $wdrl_id, 'status' => STATUS_PENDING])->first()) {
                    $dataForUpdate = [
        ...
        if ($request->ajax()) {
            ...
            return datatables()->of($withdrawal)
                ...
                ->editColumn('status', function ($wdrl) {
                    return DepositeStatus::tryFrom($wdrl->status)->statusHtml();
                })
    ...
    public function withdrawalHistoryData($status = 0)
    {
        return DB::table('withdraw_histories')->select(
            ...
            ->leftJoin('networks', 'networks.id', '=', 'withdraw_histories.network_id')
