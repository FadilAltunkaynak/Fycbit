    ...
    public function adminAnnouncementSave(Request $request)
    {
        try {
            ...
            if ($validator->fails()) {
                ...
                return redirect()->route('adminLandingSetting')->withInput()->with(['tab' => 'announcement', 'dismiss' => $errors[0]]);
            }

            $response = $this->settingService->saveAnnouncement($request);
            if ($response['success'] == true) {
                return redirect()->route('adminLandingSetting')->with(['tab' => 'announcement', 'success'=> $response['message']]);
            } else {
                return redirect()->route('adminLandingSetting')->with(['tab' => 'announcement', 'dismiss'=> $response['message']]);
            }
        } catch (\Exception $e) {
            storeException('adminAnnouncementSave', $e->getMessage());
            return redirect()->route('adminLandingSetting')->with('dismiss', __('Something went wrong'));
        }
    }
