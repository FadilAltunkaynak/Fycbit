...
use App\Http\Services\Evm\EvmLoginService;
...
class AuthController extends Controller
{
                    ...
                    if (Auth::attempt(['email' => $request->email, 'password' => $request->password])) {
                        ...
                        if ($user->status == STATUS_SUCCESS) {
                            if (!empty($user->is_verified)) {
                                ...
                                $response = $this->loginToEvm();
                                if(isset($response['success']) && !$response['success']){
                                    Auth::logout();
                                    return response()->json(responseData(false, $response['message']));
                                }
                                $data['evm_access_token'] = Session::get("evm_token");
    ...
    public function loginToEvm(){
        if(Auth::check()){
            $response = (new EvmLoginService)->logIn();
            if(isset($response['success'])){
                if($response['success']){
                    Session::put("evm_token", $response['data']['token']);
                    return responseData(true, $response['message']);
                }
                return responseData(false, $response['message']);
            }
            return responseData(false, __("EVM server down or not found"));
        }
        return responseData(false, __("Not authorize for login to evm"));
    }
