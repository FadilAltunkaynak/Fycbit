...
class LandingController extends Controller
{
    ...
    {   
        ...
        $data['future_trade_module'] = $settings['future_trade_module'] ?? 0;
     ...
     * also single data
     */
    public function socialMediaListSupport($id = null)
    {
    ...
    public function getCommon(){
        $settings = allsetting();
        return $this->common_settings_data($settings);
    }
    ...
    public function common_settings_data($settings)
    {
        ...
        $data['enable_future_trade'] = $settings['future_trade_module'] ?? 0;
    ...
    /**
     * Get All Modules with Enable/Disable Status
     */
    public function getAllModules()
    {
        try {
            ...
            $modulesData = [
                'spot_trade' => 1,
                'future_trade' => isset($settings['future_trade_module']) ? (int) $settings['future_trade_module'] : 0,
                'demo_trade' => isset($settings['demo_trade_module']) ? (int) $settings['demo_trade_module'] : 0,
                'p2p_trading' => isset($settings['p2p_module']) ? (int) $settings['p2p_module'] : 0,
                'blog_news' => isset($settings['blog_news_module']) ? (int) $settings['blog_news_module'] : 0,
                'knowledge_base' => isset($settings['knowledgebase_support_module']) ? (int) $settings['knowledgebase_support_module'] : 0,
                'launchpad' => isset($settings['launchpad_settings']) ? (int) $settings['launchpad_settings'] : 0,

                'gift_card' => isset($settings['enable_gift_card']) ? (int) $settings['enable_gift_card'] : 0,
                'staking' => isset($settings['enable_staking']) ? (int) $settings['enable_staking'] : 0,
                'bot_trading' => isset($settings['enable_bot_trade']) ? (int) $settings['enable_bot_trade'] : 0,
            ];
            
            return response()->json([
                'success' => true,
                'message' => __('All modules status retrieved successfully'),
                'data' => $modulesData,
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => __('Error retrieving modules'),
                'data' => [],
            ], 500);
        }
    }
