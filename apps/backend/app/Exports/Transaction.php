...
class Transaction implements FromCollection, WithHeadings
{
    ...
    public function collection()
    {
        try {
            ...
            if ($type == 'deposit')
                $data = DepositeTransaction::select('address_type', 'sender_wallet_id', 'coin_type', 'address', 'receiver_wallet_id', 'amount', 'fees', 'created_at')
                    ->with(['senderWallet.user', 'receiverWallet.user']);
            if ($type == 'pending_deposit')
                $data = DepositeTransaction::select('sender_wallet_id', 'address', 'from_address', 'coin_type', 'network_type', 'amount', 'transaction_id', 'created_at')
                    ->with(['senderWallet.user'])->where('status', STATUS_PENDING);
            if ($type == 'withdrawal')
                $data = WithdrawHistory::select('address_type', 'user_id', 'coin_type', 'address', 'receiver_wallet_id', 'amount', 'fees', 'transaction_hash', 'status', 'created_at')
            ...
            if ($type == 'referral') {
                $data = AffiliationHistory::
                    ...
                    ->latest()->select(
                        'affiliation_histories.transaction_id',
                        'reference_user.email as reference_user_email',
                        'referral_user.email as referral_user_email',
                        'affiliation_histories.coin_type',
                        'affiliation_histories.amount',
            ...
            $data->map(function ($data) use ($type) {
                ...
                if ($type == 'withdrawal') {
                    ...
                    $data->amount = $data->amount . ' ' . $data->coin_type;
                    $data->fees = $data->fees . ' ' . $data->coin_type;
                    $data->status = DepositeStatus::tryFrom($data->status)->statusHtml();
                }
            });
            return $data;
        } catch (\Exception $e) {
    ...
    public function headings(): array
    {
        ...
        if ($this->type == 'deposit')
            return $deposit;
        if ($this->type == 'pending_deposit')
            return $pending_deposit;
        if ($this->type == 'referral')
            return $referral;
        if ($this->type == 'pending_withdrawal_form')
            return $pending_withdrawal_form;
        if ($this->type == 'reject_withdrawal_form' || $this->type == 'active_withdrawal_form')
            return $reject_withdrawal_form;
        return $withdraw;
    }
}