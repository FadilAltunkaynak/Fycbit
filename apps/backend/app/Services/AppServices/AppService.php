...
use App\Services\CacheService\SpotCoinPairCacheService;
use App\Services\CacheService\SpotOrderBookCacheService;
use App\Services\CacheService\SpotTransactionCacheService;
...
class AppService implements AppServiceContract
{
    ...
    public static function set_dependence(Application $app): void
    {
        ...
        // Parallel Service Added To Application
        $app->bind(
            ParallelServiceContract::class,
            fn()=> new ParallelService()
        );
        $app->alias(ParallelServiceContract::class, 'parallel');
        ...
        // Spot Order Book Cache Service Added To Application
        $app->singleton(SpotOrderBookCacheService::class);
        $app->alias(SpotOrderBookCacheService::class, 'spot_orderbook_cache');

        // Spot Transaction Cache Service Added To Application
        $app->singleton(SpotTransactionCacheService::class);
        $app->alias(SpotTransactionCacheService::class, 'spot_transaction_cache');

        // Spot Coin Pair Cache Service Added To Application
        $app->singleton(SpotCoinPairCacheService::class);
        $app->alias(SpotCoinPairCacheService::class, 'spot_coinpair_cache');

        // Spot Chart Cache Service Added To Application
        $app->singleton(SpotChartCacheService::class);
        $app->alias(SpotChartCacheService::class, 'spot_chart_cache');
    }
    ...
    /**
     * This method will set custom blade directives
     * @return void
     */
    public static function set_custom_blade_directive(): void
    {
        Blade::directive('disabled', fn ($expression)=> "<?php echo filter_var($expression, FILTER_VALIDATE_BOOLEAN) ?  'disabled' : '' ; ?>");
        Blade::directive('selected', fn ($expression)=> "<?php echo filter_var($expression, FILTER_VALIDATE_BOOLEAN) ?  'selected' : '' ; ?>");
        Blade::directive('checked' , fn ($expression)=> "<?php echo filter_var($expression, FILTER_VALIDATE_BOOLEAN) ?  'checked'  : '' ; ?>");
    }
}
