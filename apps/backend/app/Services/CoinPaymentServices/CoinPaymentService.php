...
final class CoinPaymentService extends CoinPaymentApi
{
    ...
    public function getWalletAddress(string $coin_type)
    {
        ...
        if(!$wallet_id){
            try{
                $hasWallet = CoinPaymentWallet::where("coin_type", $coin_type)->whereNotNull("wallet_id")->first();
                ...
                if(! $hasWallet){
                    ...
                    $hasWallet = CoinPaymentWallet::create([
                        "coin_id" => $coin?->id ?? $networkAddress?->coin_id ?? 0,
                        "coin_type" => $coin_type,
                        "code" => $coin_type,
    ...
    public function CreateWithdrawal(int $wallet_id, string $amount, string $currency, string $address, string $dest_tag = "")
    {
        ...
        $coinPaymentWallet = CoinPaymentWallet::where('coin_type', $currency)->first();
        if(!$coinPaymentWallet || !$coinPaymentWallet?->wallet_id)
            return [ "error" => __("CoinPayment wallet id not found")];

        $walletIdStr = $coinPaymentWallet->wallet_id;
