...
//Discount Type array
function getCurrencyType($input = null)
{
    $output = [
        FIAT_TYPE => __('Fiat'),
        CRYPTO_TYPE => __('Crypto')
    ];
    if (is_null($input)) {
        return $output;
    } else {
...
function status($input = null)
{
    $output = [
        ...
        STATUS_INACTIVE => '<span class="badge badge-warning">' . __('Inactive') . '</span>',
    ];
    if (is_null($input)) {
        return $output;
    } else {
...
function api_settings($input = null)
{
    $output = [
        ...
        SOLANA_BASE_COIN => __('Solana Base Coin'),
    ];
    if (is_null($input)) {
        return $output;
    } else {
...
//social media list
function socialMediaList($input = null)
{
    $output = [
...
function ICOstatusAction($input = null)
{
    $output = [
        STATUS_PENDING => '<span class="badge badge-warning">' . __('Pending') . '</span>',
        STATUS_ACCEPTED => '<span class="badge badge-success">' . __('Accepted') . '</span>',
        STATUS_REJECTED => '<span class="badge badge-danger">' . __('Rejected') . '</span>',
        STATUS_MODIFICATION => '<span class="badge badge-warning">' . __('Modification') . '</span>',
    ];
    if (is_null($input)) {
        return $output;
    } else {
...
function getBaseNetworkType($index = null): string|array
{
    ...
    $types = [
        ...
        SOLANA_BASE_COIN => __('Solana Base'),
    ];
...
function getNetworkEnvironment($index = 'all')
{
    $types = [
        NETWORK_MAINNET => __("Mainnet"),
        NETWORK_TESTNET => __("Testnet")
    ];
    if ($index == 'all')
        return $types;
    return isset($types[$index]) ? $types[$index] : __("Not found");
}
