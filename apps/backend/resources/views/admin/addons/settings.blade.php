...
@php $update = false; @endphp
 ...
 <form action="{{route('saveAddonsSettings')}}" method="post"
        ...
        @endif
        @if(isset($FutureTrade) && $FutureTrade)
            @php $update = true; @endphp
            <div class="header-bar">
                <div class="table-title">
                    <h3>{{__('Future Trade Addons')}}</h3>
                </div>
            </div><hr>
            <div class="row">
                <div class="col-lg-6 col-12 mt-20">
                    <div class="form-group">
                        ...
                        <div class="cp-select-area">
                            <select name="future_trade_module" class="form-control">
                                <option @if(isset($settings['future_trade_module']) && $settings['future_trade_module'] == STATUS_PENDING) selected @endif value="{{STATUS_PENDING}}">{{__("Disable")}}</option>
                                <option @if(isset($settings['future_trade_module']) && $settings['future_trade_module'] == STATUS_ACTIVE) selected @endif value="{{STATUS_ACTIVE}}">{{__("Enable")}}</option>
                            </select>
                        </div>
                    </div>
                </div>
