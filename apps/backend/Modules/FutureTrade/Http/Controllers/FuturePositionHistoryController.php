...
class FuturePositionHistoryController extends Controller
{
    public function list(): mixed
    {
        if (IS_API_CALL) {
            $query = FuturePositionHistory::query()
                ->join('future_positions', 'future_positions.id', '=', 'future_position_histories.future_position_id')
                ->join('future_coin_pairs', 'future_coin_pairs.id', '=', 'future_positions.future_coin_pair_id')
                ->join('users', 'users.id', '=', 'future_position_histories.user_id')
                ->where('future_positions.status', '<>', PositionStatusEnum::CLOSED->value)
                ->select(
                    'future_position_histories.*',
                    'future_positions.uid',
                    'future_positions.order_type',
                    'future_positions.margin_mode',
                    'users.email',
            ...
            return datatables($query)
                ...
                ->addColumn('amount', fn($trade) => ($trade->amount > 0 ? trim_num($trade->amount, $trade->trade_decimal ?: 8) : trim_num('0', $trade->trade_decimal ?: 8)) . ' ' . $trade->base_coin_code ?? '')
        ...
        $data['title'] = __('Future Position Histories');
        return view('futureTrade::position_histories.list', $data);
    }

    public function getPositionHistory(Request $req, FuturePositionService $service)
    {
        if (!$req->filled('uid')) {
            return ResponseFacade::failed(__('Trade uid is required'))->send();
        }

        $order = FuturePositionHistory::query()
            ->join('future_positions', 'future_positions.id', '=', 'future_position_histories.future_position_id')
            ->join('future_coin_pairs', 'future_coin_pairs.id', '=', 'future_positions.future_coin_pair_id')
        ...
        $orderHtmlData = view('futureTrade::position_histories.components.position-history-details', ['position' => $order])->render();
