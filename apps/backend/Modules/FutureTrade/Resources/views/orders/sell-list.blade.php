...
@section('sidebar')
@include('futureTrade::layouts.sidebar',['menu'=>'sell_orders'])
@endsection
@section('content')
    ...
    <div class="custom-breadcrumb">
        <div class="row">
            <div class="col-12">
                <ul>
                    <li class="active-item">{{__('Future Sell Orders')}}</li>
                </ul>
            </div>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header border-0 pb-0">
                    <h5 class="modal-title font-weight-bold">{{__('Sell Order Details')}}</h5>
    ...
    <div id="cancel_model" class="modal fade" role="dialog">
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                ...
                <div class="modal-body"><p>{{ __('Do you want to Cancel ?')}}</p></div>
                <div class="modal-footer"><button type="button" class="btn btn-default" data-dismiss="modal">{{__("Close")}}</button>
                    <a id="cancel_request_a" class="btn btn-danger" href="#">{{__('Confirm')}} </a>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    function cancelModel(e){
        e.preventDefault();
        let url = e.target.closest('.cancel-url').dataset.url;
        document.getElementById('cancel_request_a').setAttribute('href', url);
    ...
    function getOrderDetails(uid){
        let url = '{{ route("future.order.get-modal-order") }}';
