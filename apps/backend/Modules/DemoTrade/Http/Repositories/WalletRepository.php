...
//        dd($transactions);
    ...
    public function getMyWalletListWithOnorderWithTotal($user_id,$paginate = null)
    {
        ...
        $wallets = Wallet::join($db.'coins',$db.'coins.id', '=', 'wallets.coin_id')
            ->join('demo_coins','demo_coins.coin_id', '=', 'wallets.coin_id')
            ->where(['wallets.user_id'=> $user_id, 'wallets.type'=> PERSONAL_WALLET, $db.'coins.status' => STATUS_ACTIVE])
            ->orderBy('wallets.id', 'ASC')
            ->select('wallets.*', $db.'coins.coin_icon', $db.'coins.is_withdrawal', $db.'coins.is_deposit',$db.'coins.trade_status','demo_coins.faucet_amount','demo_coins.faucet_time','demo_coins.faucet_min_balance',)
            ->get();
        if (isset($wallets[0])) {
