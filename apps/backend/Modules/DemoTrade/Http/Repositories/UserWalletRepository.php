...
class UserWalletRepository extends CommonRepository
{
    ...
    public function getUserSingleWalletBalance($userId, $coinId)
    {
        ...
        $wallet = UserWallet::select('wallets.id as wallet_id', 'wallets.*', $db.'coins.*')
            ->join($db.'coins', ['wallets.coin_type' => $db.'coins.coin_type'])
            ->where(['wallets.user_id' => $userId, 'wallets.is_primary'=>STATUS_ACTIVE, $db.'coins.id'=>$coinId])
            ->first();

        if (empty($wallet))
            $wallet = UserWallet::select('wallets.id as wallet_id', 'wallets.*', $db.'coins.*')
                ->join($db.'coins', ['wallets.coin_type' => $db.'coins.coin_type'])
                ->where(['wallets.user_id' => $userId, $db.'coins.id'=>$coinId])
