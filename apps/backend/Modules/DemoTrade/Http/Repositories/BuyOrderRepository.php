...
class BuyOrderRepository extends CommonRepository
{
    ...
    public function getAllOrders($base_coin_id, $trade_coin_id)
    {
        if (true || (env('APP_ENV') == 'local') || (env('APP_ENV') == 'dev') || (env('APP_ENV') == 'production')) {
            if(getUserId()) {
                $filterBuys = DB::connection("DemoTradeMysql")->table(DB::connection("DemoTradeMysql")->raw('(select base_coin_id,trade_coin_id, TRUNCATE(price,8) as price, visualNumberFormat(TRUNCATE(sum(amount-processed),8)) as amount, visualNumberFormat(TRUNCATE(sum((amount - processed) * price), 8)) as total, status, processed, created_at
                                        from buys
...
//                        'favourite_order_books.user_id' => DB::connection("DemoTradeMysql")->raw(getUserId()),
                        ...
                        'favourite_order_books.type' => DB::connection("DemoTradeMysql")->raw("'buy'")])
                    ->leftJoin(DB::connection("DemoTradeMysql")->raw('(select visualNumberFormat(TRUNCATE(sum(amount-processed),8)) as amount ,price from buys where user_id =' . getUserId() . ' and base_coin_id = ' . $base_coin_id . ' and trade_coin_id = ' . $trade_coin_id . ' and  status = 0 and is_market = 0 and deleted_at IS NULL group by price) t2'), ['t1.price' => 't2.price'])
    ...
    {
        return  DB::connection("DemoTradeMysql")->table('buys')
            ->where(['user_id' => $userId,'base_coin_id' => $baseCoinId, 'trade_coin_id' => $tradeCoinId, 'status' => '0', 'is_market' => 0, 'deleted_at' => null])
            ->select( DB::connection("DemoTradeMysql")->raw('SUM(TRUNCATE((amount - processed)*price,8)+ TRUNCATE((amount - processed)*price,8)*0.01*case when (maker_fees > taker_fees)  then (maker_fees) else (taker_fees) end ) as total'))
            ->get();
    }
    ...
    public function getTotalAmount($baseCoinId, $tradeCoinId)
    {
        if (true || (env('APP_ENV') == 'local') || (env('APP_ENV') == 'dev')) {
            return DB::connection("DemoTradeMysql")->table('buys')
                ...
                ->select('status', DB::connection("DemoTradeMysql")->raw('TRUNCATE(SUM((amount - processed)*price),8) as total'))->groupBy('status')
                ->get();
        }
    }
...
//        if(getUserId()) {
        if($userId) {
            return Buy::where(['user_id' => $userId, 'base_coin_id' => $baseCoinId, 'trade_coin_id' => $tradeCoinId, 'status' => 0, 'is_market' => 0])
                ->select(DB::connection("DemoTradeMysql")->raw("'buy' as type"), 'id', DB::connection("DemoTradeMysql")->raw("visualNumberFormat(price) as price"), 'created_at',
            ...
            return Buy::where(['user_id' => 0, 'base_coin_id' => $baseCoinId, 'trade_coin_id' => $tradeCoinId, 'status' => 0, 'is_market' => 0])
                ->select(DB::connection("DemoTradeMysql")->raw("'buy' as type"), 'id', DB::connection("DemoTradeMysql")->raw("visualNumberFormat(price) as price"), 'created_at', DB::connection("DemoTradeMysql")->raw('visualNumberFormat(TRUNCATE((amount - processed), 8)) as amount, visualNumberFormat(TRUNCATE((amount - processed) * price, 8)) as total,
                    (case when (maker_fees > taker_fees)
                     THEN
                          visualNumberFormat(TRUNCATE((((amount - processed) * price) * maker_fees) * 0.01, 8))
                     ELSE
    ...
    public function getOrders()
    {
        ...
        return Buy::select(DB::connection("DemoTradeMysql")->raw($db."users.email as email, base_coin_table.coin_type as base_coin, trade_coin_table.coin_type as trade_coin, price,
            ...
            ->join($db.'users',[$db.'users.id' => 'buys.user_id'])
