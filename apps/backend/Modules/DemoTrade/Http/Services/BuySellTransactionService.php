...
use Modules\DemoTrade\Jobs\StopLimitProcessJob;
use Modules\DemoTrade\Jobs\TradingViewChartJob;
use Modules\DemoTrade\Jobs\TransactionBroadcastJob;
...
use Modules\DemoTrade\Http\Repositories\TransactionRepository;
...
class BuySellTransactionService extends BaseService
{
    ...
    public function process($orderId, $orderType)
    {
        try {
            ...
            $channel_name = 'demo_trade-info-' . $order->base_coin_id . '-' . $order->trade_coin_id;
            $event_name = 'process';
    ...
    public function sendBuySellDataWhenTransactionDone($order)
    {
        ...
        $channel_name = 'demo_dashboard-' . $order->base_coin_id . '-' . $order->trade_coin_id;
    ...
    public function refundIfFeesZero($order, $type, $price)
    {
        ...
        if (bccomp($buyFees, "0") === 0 || bccomp($sellFees, "0") === 0) {
            ...
            try {
                ...
                if (bccomp(truncate_num($order->processed), "0") === 0) {
    ...
    public function refundIfFeesZeroMarket($order, $price)
    {
        ...
        if (bccomp($adjustValue, 0) !== 0 && (bccomp($buyFees, "0") === 0 || bccomp($sellFees, "0") === 0)) {
            ...
            try {
                ...
                if (bccomp(truncate_num($order->processed_request_amount), "0") === 0) {
                    $model->find($order->id)->delete();
                } else {
    ...
    public function update24HourPrice($transaction)
    {
        try {
            ...
            $tData = Transaction::select(DB::connection("DemoTradeMysql")->raw('max(price) as max'), DB::connection("DemoTradeMysql")->raw('min(price) as min'), DB::connection("DemoTradeMysql")->raw('sum(amount) as total'))
    ...
    public function closeOrder($order, $orderType)
    {
        DB::connection("DemoTradeMysql")->beginTransaction();
        try {
            if ($order->is_market == 1 && $order->status == 0 && bccomp($order->amount, truncate_num($order->processed))) {
                $amount = bcsub($order->amount, truncate_num($order->processed));
                if ($orderType == 'buy') {
    ...
    {
        $select = ['transaction_id', DB::connection("DemoTradeMysql")->raw("CASE WHEN buy_user_id =" . getUserId() . " THEN buy_fees WHEN sell_user_id =" . getUserId() . " THEN sell_fees END as fees"), DB::connection("DemoTradeMysql")->raw("visualNumberFormat(amount) as amount"), DB::connection("DemoTradeMysql")->raw("visualNumberFormat(price) as price"), DB::connection("DemoTradeMysql")->raw("visualNumberFormat(last_price) as last_price"), 'price_order_type', DB::connection("DemoTradeMysql")->raw("visualNumberFormat(total) as total"), 'created_at', DB::connection("DemoTradeMysql")->raw("TIME(created_at) as time")];
        $where = [
    ...
    public function getMyAllTransactionHistory($userId)
    {
        $select = [
            'transaction_id',
            DB::connection("DemoTradeMysql")->raw("CASE WHEN buy_user_id =" . Auth::id() . " THEN buy_fees WHEN sell_user_id =" . Auth::id() . " THEN sell_fees END as fees"),
            DB::connection("DemoTradeMysql")->raw("visualNumberFormat(amount) as amount"),
            DB::connection("DemoTradeMysql")->raw("bc.coin_type as base_coin"),
            DB::connection("DemoTradeMysql")->raw("tc.coin_type as trade_coin"),
            DB::connection("DemoTradeMysql")->raw("visualNumberFormat(price) as price"),
            DB::connection("DemoTradeMysql")->raw("visualNumberFormat(last_price) as last_price"),
            'price_order_type',
            DB::connection("DemoTradeMysql")->raw("visualNumberFormat(total) as total"),
            DB::connection("DemoTradeMysql")->raw("transactions.created_at as time")
        ];
        $where = [];
        $time = 0;
        $where['buy_user_id'] = $userId;
