...
use Modules\DemoTrade\Http\Services\UserWalletService;
...
class DashboardService
{
    ...
    public function getLastPriceList($baseCoinId = null, $tradeCoinId = null)
    {
        if ($baseCoinId == null && $tradeCoinId == null) {
            return CoinPair::orderBy('created_at', 'desc');
    ...
    public function deleteOrder($request)
    {
        ...
        try {
            ...
            $order = $service->getDocs(['id' => $id, 'user_id' => getUserId(), 'status' => 0])->first();
            if (empty($order)) {
                DBService::rollBack();
                return [
                    'status' => false,
            ...
            if ($type == 'buy') {
                ...
                $wallet = $walletRepository->getDocs(['user_id' => getUserId(), 'coin_id' => $order->base_coin_id])->first();
                $response = $walletRepository->addBalanceById(getUserId(), $order->base_coin_id, $returnAmount);
                $orderService = new BuyOrderRepository(Buy::class);
            } else {
                $wallet = $walletRepository->getDocs(['user_id' => getUserId(), 'coin_id' => $order->trade_coin_id])->first();
                $response = $walletRepository->addBalanceById(getUserId(), $order->trade_coin_id, $restAmount);
            ...
            if ($order->processed > 0) {
                $isDeleteOrUpdate = $orderService->updateWhere(['id' => $order->id, 'user_id' => getUserId(), 'status' => 0], ['status' => 1, 'amount' => $order->processed]);
            } else {
                $isDeleteOrUpdate = $orderService->deleteWhere(['id' => $order->id, 'user_id' => getUserId(), 'processed' => 0, 'status' => 0, 'deleted_at' => null]);
            }
            if (!$isDeleteOrUpdate) {
                DBService::rollBack();
                return [
