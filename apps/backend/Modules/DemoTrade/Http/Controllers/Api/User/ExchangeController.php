...
use Modules\DemoTrade\Http\Services\TradingViewChartService;
...
class ExchangeController extends Controller
{
    ...
    public function appExchangeDashboard(Request $request, $pair=null){
        ...
        if(Auth::guard('api')->check())  {
            create_coin_demo_wallet(getUserId());
        }
        ...
        if(isset($pair)) {
            ...
            if (empty($request->base_coin_id) || empty($request->trade_coin_id)) {
                ...
                if (checkDemoPair($baseCoinId,$tradeCoinId)) {
                    $request->merge([
            ...
            $request->merge([
                'base_coin_id' => get_demo_base_coin_id(),
                'trade_coin_id' => get_demo_trade_coin_id(),
            ]);
        }
        ...
        if (checkDemoPair($request->base_coin_id,$request->trade_coin_id)) {
            $pairservice = new CoinPairService();
    ...
    {
        if(Auth::guard('api')->check())  {
            $fees = calculated_demo_fee_limit(getUserId());
        } else {
            $fees = [];
        }
        return $fees;
