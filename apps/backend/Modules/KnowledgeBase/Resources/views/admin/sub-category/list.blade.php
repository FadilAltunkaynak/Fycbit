...
@section('content')
    ...
    <div class="custom-breadcrumb">
        <div class="row">
            ...
            <div class="col-3">
                <a class="btn theme-btn float-right" href="{{route('knowledgebase_subCategoryAdd')}}">{{__('Add New')}}</a>
            </div>
        </div>
    </div>
    <!-- /breadcrumb -->
                    ...
                    <div class="table-responsive">
                        <table id="table" class=" table table-borderless custom-table display text-lg-center" width="100%">
                            ...
                            <tr>
                                ...
                                <th scope="col" class="all">{{__('Sub Category Name')}}</th>
                            ...
                            @foreach($sub_category_list as $sub_category)
                                <tr>
                                    <td>
                                        {{ isset($sub_category->category)? $sub_category->category->name : __('Not Found')}}
                                    </td>
                                    <td> {{$sub_category->name}} </td>
                                    <td>
                                        @if ($sub_category->status == STATUS_ACTIVE)
                                    ...
                                    <td>
                                        <ul class="d-flex activity-menu">
                                            <li class="viewuser">
                                                <a href="{{route('knowledgebase_subCategoryEdit', $sub_category->unique_code)}}" title="{{__("Update")}}" class="btn btn-primary btn-sm">
                                                    <i class="fa fa-pencil"></i>
                                                </a>
                                            </li>
                                            
                                            <li class="viewuser">
                                                <a href="#delete1WV4d6uF6Ytu8v1Pl_{{($sub_category->unique_code)}}" data-toggle="modal" title="{{__("Delete")}}" class="btn btn-danger btn-sm">
                                                    <i class="fa fa-trash"></i>
                                                </a>
                                                <div id="delete1WV4d6uF6Ytu8v1Pl_{{($sub_category->unique_code)}}" class="modal fade delete" role="dialog">
                                                    <div class="modal-dialog modal-sm">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-footer"><button type="button" class="btn btn-default" data-dismiss="modal">{{__("Close")}}</button>
                                                                <a class="btn btn-danger"href="{{route('knowledgebase_subCategoryDelete', $sub_category->unique_code)}}">{{__('Confirm')}} </a>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
